...
Meteor.methods({
    ...
    'CrearNuevaOrderRobot':function(TIPO_CAMBIO, CANT_INVER, MON_B, MON_C, MONEDA_SALDO, MONEDA_COMISION, ID_LOTE){
        ...
        log.info("Valores recibidos CrearNuevaOrderRobot", " TIPO_CAMBIO: "+ TIPO_CAMBIO+ " CANT_INVER: "+ CANT_INVER+ " MON_B: "+ MON_B+ " MON_C: "+ MON_C+ " MONEDA_SALDO: "+ MONEDA_SALDO+ " MONEDA_COMISION: "+ MONEDA_COMISION+ " ID_LOTE: "+ ID_LOTE,'Robot');

        var fecha = new Date();
        log.info("Valor de fecha:", fecha,'Robot');
        ...
        log.info("Valor de MON_B: ", MON_B,'Robot');
        log.info("Valor de MON_C: ", MON_C,'Robot');
        log.info("Valor de MONEDA_SALDO: ", MONEDA_SALDO,'Robot');
        log.info("Valor de V_TipoOperaciont: ", V_TipoOperaciont,'Robot');

        var RecalcIverPrec = Meteor.call("CalcularIversionPromedio", TIPO_CAMBIO, MONEDA_SALDO, CANT_INVER);
        log.info(" Valor de RecalcIverPrec: ", RecalcIverPrec,'Robot');
        var TC = TiposDeCambios.findOne({ tipo_cambio : TIPO_CAMBIO })
        log.info(" Valor de TC: ", TC,'Robot');
        var MinimoInversion = TC.valor_incremento
        log.info(" Valor de MinimoInversion: ", MinimoInversion,'Robot');
        if ( parseFloat(RecalcIverPrec.MontIversionCal) >= parseFloat(MinimoInversion) ) {
            log.info(" Valor de if ( "+ parseFloat(RecalcIverPrec.MontIversionCal)+" >= "+parseFloat(MinimoInversion) +" )",'','Robot');
            var INV_REAL = RecalcIverPrec.MontRealIversionCal;

            log.info("Valor de IdTransaccionActual: ", IdTransaccionActual,'Robot');
            log.info("Valor de TIPO_CAMBIO: ", TIPO_CAMBIO,'Robot');
            log.info("Valor de TP: ", TP,'Robot');
            log.info("Valor de INV_REAL: ", INV_REAL,'Robot');
            log.info("Valor de RecalcIverPrec.MontIversionCal: ", RecalcIverPrec.MontIversionCal,'Robot');
            log.info("Valor de RecalcIverPrec.MejorPrecCal: ", RecalcIverPrec.MejorPrecCal,'Robot');
            log.info("Valor de RecalcIverPrec.comision_hbtc: ", RecalcIverPrec.comision_hbtc,'Robot');
            log.info("Valor de RecalcIverPrec.comision_mercado: ", RecalcIverPrec.comision_mercado,'Robot');
            ...
            log.info(" Valor de DatosRobot: ", DatosRobot,'Robot');


            log.info(" Estoy aca 1",'','Robot');
            do {            
                var Orden = Meteor.call('GenerarOrderRobot', DatosRobot);
                log.info(" Estoy aca 2",'','Robot');
        ...
        log.info(' Valor de Orden 1: ', Orden,'Robot');

        while( Estado_Orden !== "filled" ){
            log.info('Estoy en el while','','Robot');
            log.info(' Valor de Orden 2: ', Orden,'Robot');
            ...
            if ( Estado_Orden === "new" || Estado_Orden === "partiallyFilled" ) {
                ...
                log.info(' Estoy en  if ( Estado_Orden === "new" || Estado_Orden === "partiallyFilled" )','','Robot');
                log.info(' Valor de Orden 3: ', Orden,'Robot');
            ...
            if ( Estado_Orden === "DuplicateclientOrderId" || Estado_Orden === "suspended" || Estado_Orden === "Estado_Orden" || Estado_Orden === "expired" || Estado_Orden === "Fallido" || Estado_Orden === "canceled" || Estado_Orden === "Quantity too low" ) {
                log.info(' Estoy en if ( Estado_Orden === "DuplicateclientOrderId" || Estado_Orden === "suspended" || Estado_Orden === "Estado_Orden" || Estado_Orden === "expired" || Estado_Orden === "Fallido" || Estado_Orden === "canceled" )','','Robot');
                var V_IdHitBTC = Orden.id;
                log.info(' Valor de Orden 6: ', Orden,'Robot');
                ...
                if ( Estado_Orden === "DuplicateclientOrderId") {   
                    log.info(' Estoy en if if ( Estado_Orden === "DuplicateclientOrderId")','','Robot');
                ...
                log.info(' Valor de Orden 7: ', Orden,'Robot');
                break
            }

            if ( Estado_Orden === "errorisnotdefined" ) {
                log.info(' Valor de Orden 8: ', Orden,'Robot');
                GananciaPerdida.update( {    "Operacion.ID_LocalAct" : IdTransaccionActual, "Operacion.Id_Lote": ID_LOTE },
                                        {
                                            $set: {
                                                    Operacion : {   
                ...
                log.info(' Valor de Orden 9: ', Orden,'Robot');
                TmpTipCambioXMonedaReord.remove({ "moneda_saldo" : MONEDA_SALDO })
                break
            }

            if ( Estado_Orden === "Insufficientfunds" ) {
                log.info(" Insufficientfunds: Valor de Orden 10: ", Orden,'Robot');
                //var V_IdHitBTC = Orden.id
                ...
                const Resultado = Meteor.call("ValidarEstadoOrdenRobot", Orden)
                var Orden = Resultado
                log.info(' Valor de Orden 11: ', Orden,'Robot');
                var Estado_Orden = Resultado.status;
        ...
        log.info(' Valor de Orden 12: ', Orden,'Robot');

        if ( Estado_Orden === "filled" ) {
            log.info(" if ( Estado_Orden === filled ) : Voy a Guardar",'','Robot');
            log.info(' Valor de Orden 13: ', Orden,'Robot');
            log.info(''," if ( Estado_Orden === filled ) : Enviando "+ TIPO_CAMBIO+' '+CANT_INVER+' '+INV_REAL+' '+MON_B+' '+MON_C+' '+MONEDA_SALDO+' '+MONEDA_COMISION+' '+Orden+' '+ID_LOTE,'Robot');
            Meteor.call('GuardarOrdenRobot', TIPO_CAMBIO, CANT_INVER,  parseFloat(INV_REAL), MON_B, MON_C, MONEDA_SALDO, MONEDA_COMISION, Orden, ID_LOTE );
            log.info(" if ( Estado_Orden === filled ) : Ya guardé",'','Robot');
        }
    },
    /**/
    ...
    'GuardarOrdenRobot': function(TIPO_CAMBIO, CANT_INVER, REAL_INVER, MON_B, MON_C, MONEDA_SALDO, MONEDA_COMISION, ORDEN, ID_LOTE){
        log.info(" Valores recibidos: ", + 'TIPO_CAMBIO: '+TIPO_CAMBIO+' CANT_INVER: '+CANT_INVER+' REAL_INVER: '+REAL_INVER+' MON_B: '+MON_B+' MON_C'+MON_C+' MONEDA_SALDO: '+MONEDA_SALDO+' MONEDA_COMISION: '+MONEDA_COMISION+' ORDEN: '+ORDEN+' ID_LOTE: '+ID_LOTE,'Robot');
        var ComisionTtl = 0

        var Negociaciones = ORDEN.tradesReport
        log.info("Valor de ORDEN: ", ORDEN.tradesReport,'Robot');
        ...
        log.info(" Valor de Transaccion: ", Transaccion,'Robot');
        ...
        log.info(" Valor de FechaTradeoAnteriorMB: ", FechaTradeoAnteriorMB,'Robot');
        log.info(" Valor de SaldoTradeoAnteriorMB: ", SaldoTradeoAnteriorMB,'Robot');
        ...
        if ( MONEDA_SALDO == MON_B ) {
            ...
            log.info(''," Valor de SaldoActualCalcMB: "+ SaldoActualCalcMB+" = "+ parseFloat(SaldoTradeoAnteriorMB)+" - ("+ parseFloat(REAL_INVER)+")",'Robot');
            ...
            log.info(" Valor de SaldoActualCalcMC: ", SaldoActualCalcMC+" = "+ parseFloat(SaldoTradeoAnteriorMC)+" - ("+ parseFloat(REAL_INVER)+" - "+parseFloat(Comision)+")",'Robot');
            var EqvSaldoActualCalcMC = Meteor.call('EquivalenteDolar', MON_C, parseFloat(SaldoActualCalcMC.toFixed(9)), 2)
        ...
        log.info(" Valor de SaldoActualCalcMB: ", parseFloat(SaldoActualCalcMB.toFixed(9)),'Robot');
        log.info(" Valor de SaldoActualCalcMC: ", parseFloat(SaldoActualCalcMC.toFixed(9)),'Robot');
        ...
        log.info(" GuardarOrdenRobot: Enviando 1 ", 'EquivalenteDolar '+ MON_B+' '+parseFloat(SaldoTradeoActualMB)+' '+ 2,'Robot');
        ...
        log.info(" GuardarOrdenRobot: Enviando 2 ", 'EquivalenteDolar '+ MON_C+' '+ parseFloat(SaldoTradeoActualMC)+' '+ 2,'Robot');
        ...
        log.info(" GuardarOrdenRobot: Enviando 3 ", 'EquivalenteDolar '+MONEDA_SALDO+' '+parseFloat(REAL_INVER)+' '+2,'Robot');
        var Eqv_V_InverSaldAct = Meteor.call('EquivalenteDolar', MONEDA_SALDO, parseFloat(REAL_INVER), 2);
        log.info("Valor de Eqv_V_InverSaldAct", Eqv_V_InverSaldAct,'Robot');
        var V_Comision = parseFloat(Comision);
        log.info(" GuardarOrdenRobot: Enviando 4 ", 'EquivalenteDolar '+MONEDA_COMISION+' '+parseFloat(V_Comision)+' '+ 2,'Robot');
        ...
        if ( MONEDA_SALDO == MON_B ) {
            ...
            log.info(" GuardarOrdenRobot: Enviando 5 ", 'EquivalenteDolar, V_MonedaAdquirida('+ V_MonedaAdquirida+ ')+ SaldoMonedaAdquirida('+ parseFloat(SaldoMonedaAdquirida)+ '),'+ 2,'Robot');
            ...
            log.info( '','Eqv_V_InverSaldAnt ('+ Eqv_V_InverSaldAnt +') = REAL_INVER('+REAL_INVER+ ') * V_EquivalenciaTradeoAnteriorMB('+ V_EquivalenciaTradeoAnteriorMB + ') / SaldoTradeoAnteriorMB('+ SaldoTradeoAnteriorMB + ')','Robot');
            ...
            log.info( '','V_Ganancia ('+ V_Ganancia +') = Eqv_V_InverSaldAct('+ Eqv_V_InverSaldAct +') - Eqv_V_InverSaldAnt('+ Eqv_V_InverSaldAnt+ ')' ,'Robot');
            ...
            for (CTCR = 0, T_TiposDeCambiosResetear = TiposDeCambiosResetear.length; CTCR < T_TiposDeCambiosResetear; CTCR++) {
                ...
                log.info(" GuardarOrdenRobot: Enviando 6 ", 'EquivalenteDolar, V_MonedaAdquirida('+ V_MonedaAdquirida+ '), SaldoMonedaAdquirida('+ parseFloat(SaldoMonedaAdquirida)+ '),'+2,'Robot');
                var V_EquivSaldoMonedaAdquirida =  Meteor.call('EquivalenteDolar', V_MonedaAdquirida, parseFloat(SaldoMonedaAdquirida), 2);
                ...
                log.info( '','Eqv_V_InverSaldAnt ('+ Eqv_V_InverSaldAnt +') = REAL_INVER('+REAL_INVER+') * V_EquivalenciaTradeoAnteriorMC('+ V_EquivalenciaTradeoAnteriorMC +') / SaldoTradeoAnteriorMC('+ SaldoTradeoAnteriorMC + ')','Robot');
                var V_Ganancia = parseFloat((parseFloat(Eqv_V_InverSaldAct) - parseFloat(Eqv_V_InverSaldAnt)).toFixed(4));
                log.info( '','V_Ganancia ('+ V_Ganancia +') = Eqv_V_InverSaldAct('+ Eqv_V_InverSaldAct +') - Eqv_V_InverSaldAnt('+ Eqv_V_InverSaldAnt+')','Robot');
