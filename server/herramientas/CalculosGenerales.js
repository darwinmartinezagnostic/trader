...
Meteor.methods({
    ...
    'CalculaMultiplo':function(CANT, NUM_MULTIPLO){

        var resto = (parseFloat(CANT) % parseFloat(NUM_MULTIPLO)) / 100;

        if ( resto === 0 ) {
           var NV_CANT_INVER = CANT;
        }else {
            if (resto >= NUM_MULTIPLO / 2) {
                var NV_CANT = Math.ceil(CANT / NUM_MULTIPLO) * NUM_MULTIPLO;
            }
            else {
                var NV_CANT = Math.floor(CANT / NUM_MULTIPLO) * NUM_MULTIPLO;
            }
        }
